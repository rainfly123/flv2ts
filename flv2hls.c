...
int write_frame(AVFormatContext *s, AVPacket *pkt, AVCodecContext *avctx, AVBitStreamFilterContext *bsfc, int video);
int write_frame(AVFormatContext *s, AVPacket *pkt, AVCodecContext *avctx, AVBitStreamFilterContext *bsfc, int video){
    int ret;
...
//      av_init_packet(&packet);
        ...
        is = ic->streams[packet.stream_index];
        os = NULL;
        ...
        if (packet.pts != (int64_t)AV_NOPTS_VALUE) 
            packet.pts = av_rescale_q(packet.pts, is->time_base, os->time_base) ;
        //else
         //   packet.pts = AV_NOPTS_VALUE;
        if (packet.dts != (int64_t)AV_NOPTS_VALUE)
            packet.dts = av_rescale_q(packet.dts, is->time_base, os->time_base) ;
        //else
       ...
       ret =  write_frame(oc, &packet, video_st->codec, bsfc, packet.stream_index == video_index);
        if (ret < 0) {
